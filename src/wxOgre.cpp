...
void wxOgre::OnMouseMotion(wxMouseEvent& event)
{
    if(event.Dragging())
    {
        ...
        if(event.LeftIsDown())
        {
            ...
            mCameraNode->setOrientation(  Ogre::Quaternion( mPitch, mCamera->getRight() ) * Ogre::Quaternion( mYaw,  mCamera->getUp() )  );
